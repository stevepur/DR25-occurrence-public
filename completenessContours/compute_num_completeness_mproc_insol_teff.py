...
def read_stellar_table(filename):
    ...
    return stellar_dict, kic.kepid, kic
...
def nas_multi_grid_dr25(worker_id, n_workers, min_period, max_period,
                        ...
                        ve_model_name, output_prefix, extrap_value=None):
    ...
    stellar_dict, kiclist, kic = read_stellar_table(stellar_database_filename)
    ...
    # we're going to have nTeffBins Teff bins
    minFlux = 0.1
    maxFlux = 2.1
    nTeffBins = 10 # should be 10
    TeffMin = np.min(kic.teff)
    TeffMax = np.max(kic.teff)
    dTeff = (TeffMax - TeffMin)/nTeffBins
    nStarsTeff = np.zeros(nTeffBins)
    meanTeff = np.zeros(nTeffBins)

    period_want2d, rp_want2d = np.meshgrid(period_want, rp_want)
    shape2d = period_want2d.shape
    # 2 bins for period-radius
    # 1 bin for normal insol
    # nTeffBins bins for insol Teff dependence
    shape3d = (3+nTeffBins,) + shape2d
    ...
    for i in range(len(kiclist)):
        ...
        if np.mod(i, n_workers) == worker_id :
            ...
            curdict.extrap_value = extrap_value
            curdict.minFlux = minFlux
            curdict.maxFlux = maxFlux
            if not doneOnce:
                DEMod = None
            probdet, probtot, DEMod, probtot_insol = kp.kepler_single_comp_dr25(curdict, DEMod=DEMod)
            doneOnce = True
            ...
            if np.logical_not(np.all(np.isfinite(probtot))):
                ...
                print (curid)
                continue
            if np.logical_not(np.all(np.isfinite(probtot_insol))):
                print ("non finite probtot_insol detected.  skipping.")
                print (curid)
                continue
            cumulative_array[0] = cumulative_array[0] + probdet
            cumulative_array[1] = cumulative_array[1] + probtot
            cumulative_array[2] = cumulative_array[2] + probtot_insol
            teffBin = int(nTeffBins*(curdict.teff - TeffMin)/(TeffMax - TeffMin))
            if teffBin >= nTeffBins:
                teffBin = nTeffBins - 1
            cumulative_array[3 + teffBin] = cumulative_array[3 + teffBin] + probtot_insol
            nStarsTeff[teffBin] = nStarsTeff[teffBin] + 1
            meanTeff[teffBin] = meanTeff[teffBin] + curdict.teff
    ...
    prihdr.set('NRP',n_rp)
    prihdr.set('MINFLX',minFlux)
    prihdr.set('MAXFLX',maxFlux)
    prihdr.set('MAXTEFF',TeffMax)
    prihdr.set('MINTEFF',TeffMin)
    prihdr.set('NTEFF',nTeffBins)
    for i in range(nTeffBins):
        prihdr.set('NSTEFF' + str(i),nStarsTeff[i])
    for i in range(nTeffBins):
        prihdr.set('MEANT' + str(i),meanTeff[i]/nStarsTeff[i])
...
if __name__ == "__main__":
    ...
    parser.add_argument("extrap_value", type=float, nargs='?', default=None,
                        help="extrapolation value for insolation computation")
