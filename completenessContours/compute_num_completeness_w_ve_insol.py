    ...
    def generate_detection_eff_grid(self, rstar, cSlopeL, cSlopeS, dutyCycle, \
        ...
        for ii in range(nPeriods): # 5 bins, see sec 3.4 of the KSCI
            ...
            if ve_fit_filename != "none":
                ...
                # print("useDetEffValues: " + str(useDetEffValues))
                # print("useVetEffValues: " + str(useVetEffValues))
                useDetEffValues = useDetEffValues * useVetEffValues
                # print("new useDetEffValues: " + str(useDetEffValues))
...
#    insolation_want = np.linspace(1.2*innerHZ, 0.8*outerHZ, nper)
    insolation_want = np.linspace(data.maxFlux, data.minFlux, nper)
    period_at_insolation = period_at_earthflux(data.rstar, data.logg, data.teff, insolation_want)
    
    f = interpolate.interp2d(data.period_want, data.rp_want, probtot, fill_value=data.extrap_value);
    probtot_insol = f(period_at_insolation, data.rp_want);
    
    return probdet, probtot, DEMod, probtot_insol
